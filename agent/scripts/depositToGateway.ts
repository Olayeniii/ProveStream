...
async function depositViaCircle(amount: bigint): Promise<void> {
  ...
  const blockchain = requireEnv('CIRCLE_TREASURY_BLOCKCHAIN') as TokenBlockchain;
  ...
  console.log(
    "Both transactions are async on Circle's side — check their status via the Circle console " +
...
async function depositViaLocalSigner(amount: bigint): Promise<void> {
  const rpcUrl = requireEnv('RPC_URL');
  const chainId = Number(requireEnv('CHAIN_ID'));
  const privateKey = requireEnv('TREASURY_PRIVATE_KEY') as `0x${string}`;
